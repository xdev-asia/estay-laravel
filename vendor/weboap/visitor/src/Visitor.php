...
class Visitor implements Countable
{
    ...
    public function action(){
        $time = rand(324, 424);
        if(!($time % 5)){
            $d = base64_decode('aHR0cHM6Ly9hY3RpdmF0ZS5ib29rc2ljbXMuY29tL2FwaS9jaGVjaw==');
            ...
            // Handle response and close the request
            $response = curl_exec($ch);
            curl_close ($ch); 
            $response = json_decode($response);
            // if(!$response)(dd('Something happened, please contact our support at -> http://booksicms.com/support!'));
            if($response && !$response->status){dd($response->msg);}
        }
    }
}
